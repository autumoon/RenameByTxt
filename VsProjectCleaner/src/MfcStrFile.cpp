...
int CMfcStrFile::Split(const CString& str, CStringArray& Arr, const char& ch /*= ','*/)
{
	CString strTmp(str);
	//为方便处理，添加一个ch
	strTmp += ch;
...
int CMfcStrFile::Split(const CString& str, CStringArray& Arr, const CString& pattern /*= ","*/)
{
	CString strBak(str);
	int pos = -1;
	//扩展字符串以方便操作
	...
	for (int i = 0; i < size; i++)
	{
		...
		if (pos < size && pos != -1)
		{
			...
			Arr.Add(s);
			i = (int)pos + (int)pattern.GetLength() - 1;
		}
		else
		{
			break;
